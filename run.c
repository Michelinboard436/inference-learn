 ...
 *
 * 用法(阶段 0 的占位版本):
 *   ./run info model.safetensors   # 打印张量列表
 *   ./run model.safetensors tokenizer.bin "你的提示词"
 *
 * 现阶段只解析参数并打印，后续阶段逐步接入真正的功能。
 */
...
/* xorshift128 PRNG: 简单确定性随机数, 不依赖系统 /dev/random */
...
/* 从 logits 里采样一个 token id。
 *
 * temperature: 控制随机性
 *   = 0   → 贪心 (argmax, 完全确定)
 *   < 1   → 更确定 (尖锐分布)
 *   > 1   → 更随机 (平坦分布)
 *   做法: logits /= temperature, 再 softmax 成概率
 *
 * top_k: 只在概率最大的 K 个里采样 (截断长尾, 避免乱选)
 *   <= 0 → 不截断
 */
static int sample(float *logits, int vocab_size,
    ...
    if (temperature == 0.0f) {
        ...
        LOGI_L(CAT_SAMPLE, 0, "采样(贪心): argmax → id=%d (logit=%.4f)", best, bestv);
        return best;
    }
    ...
    /* top_k 截断: 把概率小的清零。
     * 完整实现要对概率排序取前 K, 这里用简化策略:
     * 迭代找当前最大值, 标记已选, 重复 K 次, 其余清零。 */
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr,
            ...
            "  %s fwd <model.safetensors> <token_id> [pos] [dump]\n"
            "  %s <model.safetensors> <tokenizer.bin> \"<prompt>\" "
            "[-v|-vv] [--report] [-t temp] [-k topk] [-s seed]\n\n"
            "  -v      debug 级日志 (每层摘要)\n"
            "  -vv     trace 级日志 (所有中间张量)\n"
            "  --report 运行后生成 trace_report.html\n",
            argv[0], argv[0], argv[0], argv[0]);
        return 1;
    }

    /* 全局预扫描: 在任何子命令前解析 -v / -vv / --report。
     * 这样所有子命令都能用, 而不用每个子命令单独处理。*/
    ...
    /* 报告模式自动开启 trace, 否则报告内容太少 */
    if (trace_get_report() && trace_get_level() < LOG_TRACE) {
        trace_set_level(LOG_TRACE);
    }


    /* 子命令: fwd —— 调试用。加载权重，输入一个 token id，打印 top-5 logits。
     * 用法和参考脚本见 README "数值验证" 一节。
     *   ./run fwd model.safetensors <token_id> [pos]
     *   ./run fwd model.safetensors 198            # 198 = '\n' 的 token id
     */
    ...
    /* 子命令: info —— 打开 safetensors 并列出所有张量 */
    if (strcmp(argv[1], "info") == 0) {
        ...
        const char *checks[] = {
            ...
            "model.layers.0.self_attn.o_proj.bias",   /* 应该不存在 */
            "lm_head.weight",                          /* tie_word_embeddings=true，应该不存在 */
        };
        printf("\n=== 关键校验 ===\n");
    ...
    /* 子命令: encode —— 测试 BPE 编码。
     *   ./run encode tokenizer.bin "Hello world"
     * 打印每个 token 的 id + 解码后的字符串，方便和 HF 对比。 */
    ...
    /* 生成: 先喂 prompt (prefill), 再自回归生成新 token */
    int max_new = 200;
    ...
    for (int pos = 0; pos < total_len; pos++) {
        ...
        /* 每轮 forward 前清空 trace 收集 (报告只保留最新一步) */
