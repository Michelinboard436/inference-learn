...
- 通俗解释**：把磁盘文件「映射」到内存地址空间，访问内存等于读文件，不用 `fread`。
...
- 通俗解释**：多字节数据在内存里的排列顺序。「小端」= 低位字节存在低地址。
...
- 通俗解释**：C 编译器认为 `float*` 和 `uint32_t*` 指向同一块内存是「未定义行为」。所以不能用 `*(float*)&bits` 做类型转换，要用 `memcpy`（编译器会优化成零开销）。
