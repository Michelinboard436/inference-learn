...
- **5.0** 先用真实数据看清「一个 token 进来后，到底发生了什么」
- **5.1** 给出单层 Transformer 的完整流程图
- **5.2～5.9** 逐个拆解每个算子，配原理 + 代码 + 真实数字

---

## 5.0 一个 token 的旅程：从 embedding 到 logits
...
## 5.2 算子 1：RMSNorm（归一化）

### 先看不做归一化会怎样

每一层都做矩阵乘法（QKV 投影），本质是 896 个数**累加**：
...
**右边每层归一化**：每一层开头先把上一层的输出**拉回标准大小**，再做矩阵乘法。所以不管上一层输出 85 还是 91，进入这层时都从同样起点开始——数值始终稳定。
...
**实际**：每一层内部做**两次**——attention 前一次，MLP 前一次。
...
**这就是注意力的本质**：dog 的输出不再只是 dog 自己的信息，而是**混入了 49% 的 cat 信息**。cat 和 dog 越相关（分数越高），混入的比例越大。
...
## 5.10 把整章串起来：一个 token 走完一遍 forward
