...
下面把 `net.h` 里所有术语分组讲透。每组术语都配「为什么 + 代码在哪」。
...
想象每个 token 是一个 896 格的「档案卡」，每格存一个浮点数。
...
残差流是 896 维的「主干道」。到了 MLP，先把它**扩张**到 4864 维
...
896 维的向量不方便直接做注意力（太粗）。把它切成 14 份，每份 64 维，
...
一层 Transformer 做「注意 + 前馈」一次。24 层就是做 24 次。
层数越多，模型能学到的关系越深、越抽象。
...
模型的「字典」有 151,936 个条目。每个 token 是字典里的一个词或字片段。
...
模型的「字典」。输入是 token 编号，查表得到这个词的「含义向量」。
...
入口「token → 向量」和出口「向量 → token 概率」用同一本字典。
...
「投影」就是矩阵乘法。`Wq` 把 896 维的输入「翻译」成 Query 用的格式。
...
投影矩阵做的是「旋转+缩放」，bias 做的是「平移」。
好比调音响：W 调音色，b 调音量基准。

Qwen 的特殊性：Q/K/V 投影有 bias，O 投影没有。这和 Llama 不同。
...
标准注意力是「14 个提问者各配 14 个资料柜」。GQA 是「14 个提问者共享 2 个资料柜」。
...
控制向量的「音量」不要太大或太小。防止经过多层后数值爆炸或消失。
...
在 net.h：每层有两个 RMSNorm：一个在 attention 前，一个在 MLP 前。
...
万一向量全是 0，RMS = 0，除以 0 会得到无穷大。加个极小的数兜底。
...
注意力负责「token 之间交换信息」，MLP 负责「每个 token 自己加工信息」。
...
传统激活是「一个输入过一个函数」。SwiGLU 多了一条「门控」支路：
...
比 ReLU 平滑。负数不是直接归零而是缓慢趋近零，正数近似恒等。
...
注意力本身不知道 token 的顺序（"猫咬人"和"人咬猫"对它一样）。
...
生成第 N 个词时，前 N-1 个词的 K/V 已经算过了，不用重算。
...
对话能塞多少个 token。KV Cache 的大小直接由它决定。
...
模型「思考」一次的过程。输入一个 token，一路算到 logits。
