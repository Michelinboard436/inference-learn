...
**最重要的一句话先说**：`dim=896` 表示 **每个 token 的特征向量有 896 个数**，这 896 是**特征维（列）**，**不是行数**。行数由"输入了几个 token"（seq_len）决定，和 896 完全无关。
...
- **解决什么问题**：防止向量经过 24 层矩阵乘法后数值爆炸（不归一化，22 层就涨到 228 亿，模型崩溃）
- **这 1792 个参数做什么**：
...
- **解决什么问题**：矩阵乘法只能"旋转+缩放"，加 bias 后多了"平移"能力，让投影更灵活
...
- **解决什么问题**：attention 算完后，14 个头各自输出了 64 维，拼起来是 896 维，但这是"14 个头的杂乱拼接"，需要 Wo 重新整合
...
- **解决什么问题**：attention 负责"词之间交换信息"，MLP 负责"每个词独立加工信息"——需要更大的"思考空间"
- **三个矩阵各做什么**：
...
**MLP 是绝对大头**——三个 `4864×896` 矩阵就占了 1307 万参数。因为 MLP 要把 896 维扩张到 4864 维（5.4 倍），给"思考空间"。**如果想压缩模型，MLP 是首要目标**（这也是 MoE 架构改革 MLP 的原因）。
